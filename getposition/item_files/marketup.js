MktUP_Instance = function () {
    ...
    this.makeReadOnly = function (keepAdd) {
        ...
        thatPanel.find('.grid .crud button.delete, .formgrid .crud button.delete').remove();
        thatPanel.find('[id*="§bt_grid_add"], [id*="§bt_save_form"]').hide();

        //thatPanel.find('.grid a, .formgrid a').css('cursor', 'default');

        thatPanel.find(".datepicker").datepicker("destroy");

        if (Mktup.getTopRender().isSheet) {
            $('.ui-dialog-buttonset button:first').remove();
            $('.ui-dialog-buttonset button span').text('fechar');
        }
    ...
    this.customModal = function (modal_title, modal_html, modal_buttons, close_function) {
        ...
        $("#" + CustomID).dialog({
            ...
            modal: true,
            close: function (event, ui) {
                if (close_function) close_function($(this));
                $("#" + CustomID).remove();
                if ($('.ui-dialog:visible').length < 1) $('html').css({ 'overflow-x': 'hidden', 'overflow-y': 'scroll' });
            },
            open: function (event, ui) {
    ...
    this.advancedSearch = function () {
        ...
        if (container_filter.is(":visible")) {
            ...
            $(container_filter).find('ul li:last button').unbind('click').click(function () {
                ...
                var not_element = $('.advancedSearchBox input[class*="inputRadio"]').not(':checked');
                for (i in not_element) {
                    objects_search = $.grep(objects_search, function (el) { return (el != not_element[i]) });
                }
            ...
            $('.advancedSearchBox').formvalidator();
        }
    };
