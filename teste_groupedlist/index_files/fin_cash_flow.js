...
buildFinCashFlowHeader = function (panelRender) {
    ...
    markup += '		</div>';
    markup += '	</div>';
    markup += '	<div class="buttonPanel"><div class="legenda" style="float: left; padding-left: 67px;"><div style="width:15px; height:15px; background-color:#dfac45; margin-top:5px; display: inline-table;"></div><span style="margin-right: 10px; display: inline-block;font-size: 12px; font-weight: bold; text-indent: 5px; color: #dfac45;">Previsto</span><div style="width:15px; height:15px; background-color:#c7c7c7; margin-top:5px; display: inline-table;"></div><span style="display: inline-block;font-size: 12px; font-weight: bold; text-indent: 5px; color: #c7c7c7;">Realizado</span></div>';
    markup += '	</div>';
    markup += '</div>';
...
bindClickToViewList = function (panelRender) {
    ...
    panelRender.find(".cf_leftside div.hasChild").click(function () {
        var that = $(this);
        var childType = panelRender.find('.cf_navigation .title span').hasClass('monthview') ? '.month' : '.day';
        ...
        if (that.hasClass("open")) {
            ...
            SyncGetDataOn(params, 'fin_cash_flow', 'getDetailSheet', false).done(function (a_result) {
                ...
                if (childType == '.month') {
                    console.log('aki');
                    $(".isChild[data-id-parent='" + that.attr("data-id-parent") + "']").find('.isChild.day').remove();
                    $(".isChild[data-id-parent='" + that.attr("data-id-parent") + "']").find('.isChild.month').show();
                }
                that.loader('end');
